...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		$query = "
		SELECT * FROM attendance_join_view
		WHERE v = '".$_SESSION["teacher_id"]."' AND (
		";

		if(isset($_POST["search"]["value"]))
		{
			$query .= '
			student_name LIKE "%'.$_POST["search"]["value"].'%" 
			OR student_roll_number LIKE "%'.$_POST["search"]["value"].'%" 
			OR attendance_status LIKE "%'.$_POST["search"]["value"].'%" 
			OR attendance_date LIKE "%'.$_POST["search"]["value"].'%") 
			';
		}
	...
	if($_POST["action"] == "fetchclasstudents")
	{ 
				
		$query = "
                SELECT * FROM tbl_student 
                WHERE student_grade_id = '".$_POST["grade"]."'
                ";
	...
	if($_POST["action"] == "Add")
	{
		...
		{
			$grade_id = $_POST["teacher_grade_id"];
		}
