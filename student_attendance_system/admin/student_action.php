...
if(isset($_POST["action"]))
{
	...
	{
		$query = "
		SELECT * FROM student_join_view
		";

		if(isset($_POST["search"]["value"]))
		{
			$query .= '
			WHERE student_join_view.student_name LIKE "%'.$_POST["search"]["value"].'%" 
			OR student_join_view.student_roll_number LIKE "%'.$_POST["search"]["value"].'%" 
			OR student_join_view.student_dob LIKE "%'.$_POST["search"]["value"].'%" 
			OR student_join_view.grade_name LIKE "%'.$_POST["search"]["value"].'%" 
			';
		}
		...
		{
			$query .= '
			ORDER BY student_join_view.student_id DESC 
			';
		}
		if($_POST["length"] != -1)
		{
