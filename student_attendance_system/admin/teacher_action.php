...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		{
			$sub_array = array();
			$timage=$row["teacher_image"];
			if($row["teacher_image"]=='')
				$timage="default.jpg";
			
			$sub_array[] = '<img src="teacher_image/'.$timage.'" class="img-thumbnail" width="75" />';
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		else
		{
			...
			if($_POST["action"] == "Edit")
			{
				...
				$query = "
				UPDATE tbl_teacher a, (select @username := 'admin') b
				SET a.teacher_name = :teacher_name, 
				a.teacher_address = :teacher_address,  
				a.teacher_grade_id = :teacher_grade_id, 
				a.teacher_qualification = :teacher_qualification, 
				a.teacher_doj = :teacher_doj, 
				a.teacher_image = :teacher_image
